  ...
  const fetchUsers = async (isManual = false) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (isManual) {
          setStatusMessage({ type: "success", text: "Customer database refreshed successfully!" });
        }
      } else {
  ...
  return (
    <div className="users-page dashboard-page-container">
      <div className="users-page-header">
        ...
        <button className={`m3-btn m3-btn-tonal refresh-btn ${loading ? "loading" : ""}`} onClick={() => fetchUsers(true)} disabled={loading}>
